...
class ImgLearnableEncoder(nn.Module):
    def __init__(self, model_cfg):
        ...
        self.learnable['imgFC'] = FakeTransformer(model_cfg.IMG_FEATURE_DIM, model_cfg.IMG_FEATURE_DIM,
                                                  model_cfg.IMG_FEATURE_DIM)
        img_encoder_layer = nn.TransformerEncoderLayer(d_model=model_cfg.IMG_FEATURE_DIM,
                                                       nhead=model_cfg.IMG_TRANSFORMER_HEAD)
    ...
    def forward(self, imgFea, maskImages, image_boxs):
        ...
        imgFea = self.learnable['imgAtt'](imgFea.transpose(0, 1), src_key_padding_mask=(maskImages == 0)).transpose(0,
                                                                                                                    1)
...
class TextLearnableEncoder(nn.Module):
    def __init__(self, model_cfg):
        ...
        self.learnable = nn.ModuleDict()
        self.learnable['textFC'] = FakeTransformer(model_cfg.TEXT_FEATURE_DIM, model_cfg.IMG_FEATURE_DIM,
                                                   model_cfg.IMG_FEATURE_DIM)
        text_encoder_layer = nn.TransformerEncoderLayer(d_model=model_cfg.TEXT_FEATURE_DIM,
                                                        nhead=model_cfg.TEXT_TRANSFORMER_HEAD)
        self.learnable['textAtt'] = nn.TransformerEncoder(text_encoder_layer,
                                                          num_layers=model_cfg.TEXT_TRANSFORMER_LAYER)
    ...
    def forward(self, textFea, maskTexts):
        ...
        # print(1)
        textFea = self.learnable['textAtt'](textFea.transpose(0, 1), src_key_padding_mask=(maskTexts == 0)).transpose(0,
                                                                                                                      1)
        # print(textFea.shape) # torch.Size([75, 80, 1024])
        # print(2)
...
class VL_model(nn.Module):
    ...
    def init_param(self):
        ...
        for param_q, param_k in zip(self.learnable['imgencoder'].parameters(),
                                    self.learnable['imgencoder_mom'].parameters()):
            param_k.data.copy_(param_q.data)  # initialize
            param_k.requires_grad = False  # not update by gradient

        for param_q, param_k in zip(self.learnable['textencoder'].parameters(),
                                    self.learnable['textencoder_mom'].parameters()):
            param_k.data.copy_(param_q.data)  # initialize
            param_k.requires_grad = False  # not update by gradient
    ...
    def forward(self, imgFea, texts, maskImages, maskTexts,
                text_lens, image_boxs, is_training=True,
                schema='image+text'):

        if self.model_cfg.IS_EXTRACT and schema == 'image+text':
            return self.extract_image_text_feature(imgFea, texts, maskImages, maskTexts, image_boxs)
        if self.model_cfg.IS_EXTRACT and schema == 'image':
            return self.extract_image_feature(imgFea, maskImages, image_boxs)
        if self.model_cfg.IS_EXTRACT and schema == 'text':
            return self.extract_text_feature(texts, maskTexts)
    ...
    def extract_image_text_feature(self, img, texts, maskImages, maskTexts, image_boxs):

        imgFea = self.learnable['imgencoder'](img, maskImages, image_boxs)  # <bsz, img_dim>
        imgFea = F.normalize(imgFea, p=2, dim=-1)
    ...
    def extract_image_feature(self, img, maskImages, image_boxs):
    ...
    def extract_text_feature(self, texts, maskTexts):
        textFea = self.learnable['textencoder'](texts, maskTexts)  # <bsz, img_dim>
        textFea = F.normalize(textFea, p=2, dim=-1)
        return textFea
